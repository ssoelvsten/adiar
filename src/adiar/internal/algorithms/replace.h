...
//
  ...
  //calculate form BDD levels and m what levels should be sweeped - maybe shouldn't be in this file??
  template<typename Policy>
  std::vector<int>
  ...
  replace_cor_scan_level(const In& in, 
                         PQ1& pq1,
                         PQ2& pq2)
    //we assume that pq has been preloaded when given here.. 
    //do this in playground for now..                     
  {
     // Set up output
    ...
    while(!pq1.empty()){
      ...
      while(!pq1.empty_level() || !pq2.empty()){
        if(debug_enabled) std::cout << "next init in level\n";
        cor_req_t<1> r;
        if (pq1.can_pull()) {
      ...
      //update level info??
      auto l = level_info(label, id+1);
      if(debug_enabled) std::cout << "adding level info: " << l << "\n";
      if (id >= 0) { aw.push(l); }
  }
